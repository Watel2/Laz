...
PRINTSWWW="/data/adb/modules/Symbiote/common/devicesname.sh"
PRINTSDEV="/data/adb/modules/Symbiote/common/devicesname2.sh"
	...
	do
		if [ -z "$INPUT5" ]; then
			...
			if  [ "$2" != "reset-script" ] && [ "$2" != "reboot" ]; then
				if [ "$2" == "update" ]; then
					echo "${N}Device berhasil di ubah.${N}"
					echo ""
				fi
				echo ""
			fi
			...
			if [ "$2" == "p" ] || [ "$2" == "r" ] || [ "$2" == "reset-script" ]; then
			    ...
			    echo -e "${R} เปลี่ยนเวอร์ชันระบบปฏิบัติการ, Fingerprint, การแสดงผล${N}"
				echo -e " ${Y}[6  ]${N} ${W}แอนดรอยด์ 6.0    ${N}${Y} [10 ]${N} ${W}แอนดรอยด์ 10 ${N}"
				echo -e " ${Y}[7  ]${N} ${W}แอนดรอยด์ 7.0    ${N}${Y} [11 ]${N} ${W}แอนดรอยด์ 11 ${N}"
				echo -e " ${Y}[8  ]${N} ${W}แอนดรอยด์ 8.0.0  ${N}${Y} [12 ]${N} ${W}แอนดรอยด์ 12 ${N}"
				echo -e " ${Y}[9  ]${N} ${W}แอนดรอยด์ 9      ${N}${Y} [12L]${N} ${W}แอนดรอยด์ 12L ${N}"
				echo -e ""
			    echo -e "${C} Tools${N}"
				echo -e " ${Y}[b]${N} ${W}สุ่มแบรนด์ ${N}"
				echo -e " ${Y}[t]${N} ${W}ลบแคช, ไฟล์ขยะ ${N}"
				GAPPSSTAT
				echo -e " ${Y}[d]${N} ${W}การจำลองอุปกรณ์ มหาเทพ${N} ${G}(New Trick) ${N}"
				echo -e " ${Y}[a]${N} ${W}สุ่มและเปลี่ยนเป็นระบบ (บ่เวิร์ค)${N} ${G}(New Trick) ${N}"
				echo -e " ${Y}[f]${N} ${W}รีเซ็ตโฟลเดอร์ 0 ${N}"
				echo -e " ${Y}[u]${N} ${W}รีเซ็ตโฟลเดอร์ 0 โดยไม่ต้องลบวิดเจ็ต ${N}"
				echo -e " ${Y}[r]${N} ${W}ลบ SSAID (เพื่อสร้างใหม่) ${N}"
				echo -e " ${Y}[z]${N} ${W}Fuck! Lazada ${N}"
				echo ""
			    echo -e "${C} รีบูตเพื่อเปิดใช้งานการเปลี่ยนแปลง${N}"
				echo -e " ${G}[y]${N} ${W}ใช่, รีบูต ${N}"
				echo -e " ${G}[n]${N} ${W}ไม่, กลับ ${N}"
				echo -e " ${G}[e]${N} ${W}จบการทำงาน ${N}"
				echo -e " ${G}[c]${N} ${W}รีบูต + ดาลวิค  ${N}${Y}(fox)${N}"
				echo ""
				echo ""
				echo -en "${Y}ป้อนตัวเลือก ${N}: "
				INV1=3
			fi
			read -r INPUT5
		fi
	...
	case $2 in
		...
		*)
			...
			if [ "$CNTTEST" == "true" ]; then
				...
				MODPROPWWW="https://raw.githubusercontent.com/Magisk-Modules-Repo/Symbiote/master/module.prop"
				module_v_ctrl
...
wipegmsgsf()   {
            echo ""
            echo -e "${Y}Wipe GMS/SF กำลังทำงาน...${N}"
            echo ""
            sleep 2
            if [ ! -d /data/data/com.google.android.gms ]; then 
            echo -e "${R}ไม่ได้ติดตั้ง GAPPS${N}"
            else
...
randombrand(){
            echo -e "${Y}สุ่มยี่ห้อ....${N}"
            echo -e "${Y}กำลังแก้ไข..${N}"
            sleep 2
            FUXB=$(head -3 /dev/urandom | tr -cd '0-9A-Za-z' | cut -c -5)
            sed -i "/.*brand=/ s/$/$FUXB/" /data/adb/modules/Symbiote/system.prop
	        reboot_fn
            }
            
GAPPSSTAT()    {
            if [ ! -d /data/data/com.google.android.gms ]; then เพิ่มการจำลอง
            echo -e " ${Y}[ ]${N} ${W}hapus data GMS/SF ${R}(Gapps Off)${N}"
            else
            echo -e " ${Y}[h]${N} ${W}hapus data GMS/SF ${G}(Gapps On)${G}"
            fi
            }
            
resetfolder() {
            echo "️"
            echo -e "${Y}รีเซ็ตโฟลเดอร์ 0${N}"
            sleep 0.5
            echo -e "${Y}สุ่มรหัส Android${N}"
            echo "️"
            sleep 1
            su -c rm -rf /data/system/users/0
            echo -e "${Y}เสร็จเรียบร้อย${N}"
            sleep 1
            reboot_fn
            }
...
resetfolderwidget() {
            echo "️"
            echo -e "${Y}รีเซ็ตโฟลเดอร์ 0 โดยไม่ต้องลบวิดเจ็ต${N}"
            sleep 0.5
            echo -e "${Y}สุ่มรหัส Android${N}"
            sleep 0.5
            echo "️"
            ...
            find /data/system/users/0/ -maxdepth 1 -type f ! -name "appwidgets.xml" ! -exec rm -r {} \;
            find /data/system/users/0/ -maxdepth 1 -type f ! -name "appwidgets.xml" ! -exec rm -r {} \;
            find /data/system/users/0/ -maxdepth 1 -type f ! -name "appwidgets.xml" ! -exec rm -r {} \;
            echo ""
            echo -e "${Y}เสร็จเรียบร้อย${N}"
...
CONVERTA6() {
            sed -i 's/release=.*/release=6.0/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:6.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:6.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:6.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:6.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:6.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 6.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 6.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 6.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 6.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 6.0 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 6"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
	        reboot_fn
            }
CONVERTA7() {
            sed -i 's/release=.*/release=7.0/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:7.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:7.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:7.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:7.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:7.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 7.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 7.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 7.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 7.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 7.0 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 7"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA8() {
            sed -i 's/release=.*/release=8.0.0/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:8.0.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:8.0.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:8.0.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:8.0.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:8.0.0\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 8.0.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 8.0.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 8.0.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 8.0.0 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 8.0.0 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 8"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA9() {
            sed -i 's/release=.*/release=9/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:9\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:9\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:9\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:9\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:9\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 9 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 9 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 9 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 9 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 9 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 9"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA10() {
            sed -i 's/release=.*/release=10/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:10\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:10\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:10\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:10\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:10\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 10 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 10 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 10 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 10 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 10 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 10"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA11() {
            sed -i 's/release=.*/release=11/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:11\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:11\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:11\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:11\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:11\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 11 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 11 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 11 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 11 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 11 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 11"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA12() {
            sed -i 's/release=.*/release=12/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:12\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:12\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:12\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:12\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:12\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 12 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 12 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 12 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 12 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 12 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 12"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
CONVERTA12L() {
            sed -i 's/release=.*/release=12.1/' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:9\//:12.1\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:10\//:12.1\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:11\//:12.1\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12\//:12.1\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/:12.1\//:12.1\//' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 9 / 12.1 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 10 / 12.1 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 11 / 12.1 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12 / 12.1 /' /data/adb/modules/Symbiote/system.prop
            sed -i 's/ 12.1 / 12.1 /' /data/adb/modules/Symbiote/system.prop
	        sleep 2
	        echo -e "แปลงเป็น Android เรียบร้อยแล้ว 12L"
	        sleep 1	
	        echo -e "กลับไปยังหน้าก่อนหน้า"
	        sleep 1	
	        clear
...
autodalvik() {
          echo -e "wipe cache\nwipe dalvik" > /cache/recovery/openrecoveryscript
          sleep 1
          reboot recovery
             }
...
copytobuildprop
echo -e ""
echo -e "${G}กระบวนการเสร็จสมบูรณ์${N}"
sleep 1
reboot_fn
}

# Cleaning system.prop
antibootloopsdk() {
CEKSDK=$(getprop ro.build.version.sdk)
sed -i "s/build.version.sdk=.*/build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/system.build.version.sdk=.*/system.build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/vendor.build.version.sdk=.*/vendor.build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/product.build.version.sdk=.*/product.build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/odm.build.version.sdk=.*/odm.build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/system_ext.build.version.sdk=.*/system_ext.build.version.sdk=$CEKSDK/g" /data/adb/modules/Symbiote/system.prop
}

randsystemprop() {
FU0=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU1=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU2=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU3=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU4=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU5=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU6=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU7=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU8=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
FU9=$(head -3 /dev/urandom | tr -cd '0-9A-Z' | cut -c -1)
sed -i 's/sdk=23/sdk=RELEASESDKM/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=24/sdk=RELEASESDKN/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=26/sdk=RELEASESDKO/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=28/sdk=RELEASESDKP/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=29/sdk=RELEASESDKQ/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=30/sdk=RELEASESDKR/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=31/sdk=RELEASESDKS/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=32/sdk=RELEASESDKL/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=6.0/release=RELEASEM/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=7.0/release=RELEASEN/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=8.0.0/release=RELEASEO/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=9/release=RELEASEP/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=10/release=RELEASEQ/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=11/release=RELEASER/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=12/release=RELEASES/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=12.1/release=RELEASEL/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:6.0\//:RELEASEM\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:7.0\//:RELEASEN\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:8.0.0\//:RELEASEO\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:9\//:RELEASEP\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:10\//:RELEASEQ\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:11\//:RELEASER\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:12\//:RELEASES\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:12.1\//:RELEASEL\//g' /data/adb/modules/Symbiote/system.prop
sed -i "s/ 6.0 / RELEASEM /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 7.0 / RELEASEN /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 8.0 / RELEASEO /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 9 / RELEASEP /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 10 / RELEASEQ /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 11 / RELEASER /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 12 / RELEASES /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ 12.1 / RELEASEL /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/0/$FU0/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/1/$FU1/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/2/$FU2/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/3/$FU3/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/4/$FU4/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/5/$FU5/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/6/$FU6/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/7/$FU7/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/8/$FU8/g" /data/adb/modules/Symbiote/system.prop
sed -i "s/9/$FU9/g" /data/adb/modules/Symbiote/system.prop
sed -i '/ro.product.board=/d' /data/adb/modules/Symbiote/system.prop
echo -e "ro.product.board=sdm$(head -3 /dev/urandom | tr -cd '6-9' | cut -c -2)0" >> /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKM/sdk=23/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKN/sdk=24/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKO/sdk=26/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKP/sdk=28/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKQ/sdk=29/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKR/sdk=30/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKS/sdk=31/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/sdk=RELEASESDKL/sdk=32/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEM/release=6.0/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEN/release=7.0/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEO/release=8.0.0/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEP/release=9/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEQ/release=10/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASER/release=11/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASES/release=12/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/release=RELEASEL/release=12.1/g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEM\//:6.0\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEN\//:7.0\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEO\//:8.0.0\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEP\//:9\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEQ\//:10\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASER\//:11\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASES\//:12\//g' /data/adb/modules/Symbiote/system.prop
sed -i 's/:RELEASEL\//:12.1\//g' /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASEM / 6.0 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASEN / 7.0 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASEO / 8.0 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASEP / 9 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASEQ / 10 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASER / 11 /g" /data/adb/modules/Symbiote/system.prop
sed -i "s/ RELEASES / 12 /g" /data/adb/modules/Symbiote/system.prop
...
copytobuildprop(){
mount -o rw,remount /
PATHSYMB=/data/adb/modules/Symbiote/system.prop
PATHSPRO=/system/build2.prop
PATHEPRO=/system/system_ext/build2.prop
PATHETCPRO=/system/system_ext/etc/build2.prop
PATHPPRO=/system/product/build2.prop
sed -i 's/\//ilovefufufu/g' $PATHSYMB
MYFINGERPRINT=$(grep -o -i -m1 "fingerprint=.*" $PATHSYMB)
MYBRAND=$(grep -o -i -m1 "brand=.*" $PATHSYMB)
MYNAME=$(grep -o -i -m1 "name=.*" $PATHSYMB)
MYDEVICE=$(grep -o -i -m1 "device=.*" $PATHSYMB)
MYBUILDID=$(grep -o -i -m1 "build.id=.*" $PATHSYMB)
MYINCREMENTAL=$(grep -o -i -m1 "incremental=.*" $PATHSYMB)
MYMANUFACTURER=$(grep -o -i -m1 "manufacturer=.*" $PATHSYMB)
MYMODEL=$(grep -o -i -m1 "model=.*" $PATHSYMB)
MYDESCRIPTION=$(grep -o -i -m1 "description=.*" $PATHSYMB)
MYDISPLAYID=$(grep -o -i -m1 "display.id=.*" $PATHSYMB)
MYBOARD=$(grep -o -i -m1 "board=.*" $PATHSYMB)
MYUSER=$(grep -o -i -m1 "user=.*" $PATHSYMB)
MYPRODUCT=$(grep -o -i -m1 "product=.*" $PATHSYMB)
MYFLAVOR=$(grep -o -i -m1 "flavor=.*" $PATHSYMB)
MYHOST=$(grep -o -i -m1 "host=.*" $PATHSYMB)
MYRELEASE=$(grep -o -i -m1 "release=.*" $PATHSYMB)

if [ ! -f $PATHSPRO ]; then 
            echo -e "NO PROPS"
            else
sed -i "s/build.fingerprint=.*/build.${MYFINGERPRINT}/g" $PATHSPRO
sed -i "s/brand=.*/${MYBRAND}/g" $PATHSPRO
sed -i "s/system.name=.*/system.${MYNAME}/g" $PATHSPRO
sed -i "s/device=.*/${MYDEVICE}/g" $PATHSPRO
sed -i "s/build.id=.*/${MYBUILDID}/g" $PATHSPRO
sed -i "s/incremental=.*/${MYINCREMENTAL}/g" $PATHSPRO
sed -i "s/manufacturer=.*/${MYMANUFACTURER}/g" $PATHSPRO
sed -i "s/system.model=.*/system.${MYMODEL}/g" $PATHSPRO
sed -i "s/description=.*/${MYDESCRIPTION}/g" $PATHSPRO
sed -i "s/display.id=.*/${MYDISPLAYID}/g" $PATHSPRO
sed -i "s/board=.*/${MYBOARD}/g" $PATHSPRO
sed -i "s/user=.*/${MYUSER}/g" $PATHSPRO
sed -i "s/product=.*/${MYPRODUCT}/g" $PATHSPRO
sed -i "s/flavor=.*/${MYFLAVOR}/g" $PATHSPRO
sed -i "s/host=.*/${MYHOST}/g" $PATHSPRO
sed -i "s/release=.*/${MYRELEASE}/g" $PATHSPRO
sed -i 's/ilovefufufu/\//g' $PATHSPRO
fi

if [ ! -f $PATHEPRO ]; then 
            echo -e "NO PROPS"
            else
sed -i "s/build.fingerprint=.*/build.${MYFINGERPRINT}/g" $PATHEPRO
sed -i "s/brand=.*/${MYBRAND}/g" $PATHEPRO
sed -i "s/system_ext.name=.*/system_ext.${MYNAME}/g" $PATHEPRO
sed -i "s/device=.*/${MYDEVICE}/g" $PATHEPRO
sed -i "s/build.id=.*/${MYBUILDID}/g" $PATHEPRO
sed -i "s/incremental=.*/${MYINCREMENTAL}/g" $PATHEPRO
sed -i "s/manufacturer=.*/${MYMANUFACTURER}/g" $PATHEPRO
sed -i "s/system_ext.model=.*/system_ext.${MYMODEL}/g" $PATHEPRO
sed -i "s/description=.*/${MYDESCRIPTION}/g" $PATHEPRO
sed -i "s/display.id=.*/${MYDISPLAYID}/g" $PATHEPRO
sed -i "s/board=.*/${MYBOARD}/g" $PATHEPRO
sed -i "s/user=.*/${MYUSER}/g" $PATHEPRO
sed -i "s/product=.*/${MYPRODUCT}/g" $PATHEPRO
sed -i "s/flavor=.*/${MYFLAVOR}/g" $PATHEPRO
sed -i "s/host=.*/${MYHOST}/g" $PATHEPRO
sed -i "s/release=.*/${MYRELEASE}/g" $PATHEPRO
sed -i 's/ilovefufufu/\//g' $PATHEPRO
fi

if [ ! -f $PATHETCPRO ]; then 
            echo -e "NO PROPS"
            else
sed -i "s/build.fingerprint=.*/build.${MYFINGERPRINT}/g" $PATHETCPRO
sed -i "s/brand=.*/${MYBRAND}/g" $PATHETCPRO
sed -i "s/system_ext.name=.*/system_ext.${MYNAME}/g" $PATHETCPRO
sed -i "s/device=.*/${MYDEVICE}/g" $PATHETCPRO
sed -i "s/build.id=.*/${MYBUILDID}/g" $PATHETCPRO
sed -i "s/incremental=.*/${MYINCREMENTAL}/g" $PATHETCPRO
sed -i "s/manufacturer=.*/${MYMANUFACTURER}/g" $PATHETCPRO
sed -i "s/system_ext.model=.*/system_ext.${MYMODEL}/g" $PATHETCPRO
sed -i "s/description=.*/${MYDESCRIPTION}/g" $PATHETCPRO
sed -i "s/display.id=.*/${MYDISPLAYID}/g" $PATHETCPRO
sed -i "s/board=.*/${MYBOARD}/g" $PATHETCPRO
sed -i "s/user=.*/${MYUSER}/g" $PATHETCPRO
sed -i "s/product=.*/${MYPRODUCT}/g" $PATHETCPRO
sed -i "s/flavor=.*/${MYFLAVOR}/g" $PATHETCPRO
sed -i "s/host=.*/${MYHOST}/g" $PATHETCPRO
sed -i "s/release=.*/${MYRELEASE}/g" $PATHETCPRO
sed -i 's/ilovefufufu/\//g' $PATHETCPRO
fi

if [ ! -f $PATHPPRO ]; then 
            echo -e "NO PROPS"
            else
sed -i "s/build.fingerprint=.*/build.${MYFINGERPRINT}/g" $PATHPPRO
sed -i "s/brand=.*/${MYBRAND}/g" $PATHPPRO
sed -i "s/product.name=.*/product.${MYNAME}/g" $PATHPPRO
sed -i "s/device=.*/${MYDEVICE}/g" $PATHPPRO
sed -i "s/build.id=.*/${MYBUILDID}/g" $PATHPPRO
sed -i "s/incremental=.*/${MYINCREMENTAL}/g" $PATHPPRO
sed -i "s/manufacturer=.*/${MYMANUFACTURER}/g" $PATHPPRO
sed -i "s/product.model=.*/product.${MYMODEL}/g" $PATHPPRO
sed -i "s/description=.*/${MYDESCRIPTION}/g" $PATHPPRO
sed -i "s/display.id=.*/${MYDISPLAYID}/g" $PATHPPRO
sed -i "s/board=.*/${MYBOARD}/g" $PATHPPRO
sed -i "s/user=.*/${MYUSER}/g" $PATHPPRO
sed -i "s/product=.*/${MYPRODUCT}/g" $PATHPPRO
sed -i "s/flavor=.*/${MYFLAVOR}/g" $PATHPPRO
sed -i "s/host=.*/${MYHOST}/g" $PATHPPRO
sed -i "s/release=.*/${MYRELEASE}/g" $PATHPPRO
sed -i 's/ilovefufufu/\//g' $PATHPPRO
fi
...
system_prop() {
	...
	echo -e "#!/system/bin/sh \nsu -c cp /storage/emulated/0/fufufu/${SIMRELEASE}_${SIMBRAND// /}_${SIMMODEL// /}_${SIMID// /}.sh /data/adb/modules/Symbiote/system.prop \nsleep 2 \nsu -c am force-stop com.xbackup.evolution \n# This file will be read by resetprop\n# MagiskHide Props Config\n# Copyright (c) 2018-2021 Didgeridoohan @ XDA Developers\n# Licence: MIT\n# Moded by fufufu \n" > $MODPATH/system.prop
...
settings_placement() {
	...
	if [ -f "$LATEFILE" ] && [ -d "$MODULESPATH/Symbiote" ]; then
...
script_install() {
	...
	placeholder_update $MODPATH/system/$BIN/fufufu ADBPATH ADB_PLACEHOLDER "$ADBPATH"
	placeholder_update $MODPATH/system/$BIN/fufufu LATEFILE LATE_PLACEHOLDER "$LATEFILE"
...
dssaid(){
su -c rm -rf /data/system/users/0/settings_ssaid.*
echo -e "DELETE SSAID เสร็จเรียบร้อย"
}
